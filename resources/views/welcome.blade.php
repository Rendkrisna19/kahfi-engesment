            ...
            <div class="relative w-full overflow-hidden">
                ...
                <div class="animate-marquee gap-6 items-center py-2">
                    ...
                    @foreach($allClients as $client)
                        <div class="px-6 py-3 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center shrink-0 hover:border-blue-500 hover:bg-white transition shadow-sm h-14 min-w-[120px]">
                            @if($client->image_url)
                                <img src="{{ $client->image_url }}" alt="{{ $client->title ?? 'Client' }}" class="h-8 max-h-8 max-w-[120px] object-contain">
                            @else
                                <span class="font-benzin text-xs font-bold text-slate-700 tracking-wide">{{ $client->title }}</span>
                            @endif
                        </div>
                    @endforeach
                </div>
        ...
        <section id="portofolio" class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
                @foreach($portfolios as $portfolio)
                    <div class="rounded-2xl bg-slate-900 border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl hover:border-blue-600 transition duration-300 group relative">
                        <div class="relative w-full aspect-[2/3] bg-slate-950 overflow-hidden flex items-center justify-center">
                            ...
                            @else
                                <div class="w-full h-full bg-slate-900 flex flex-col items-center justify-center p-4 text-center text-white">
                                    <div class="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center mb-2 text-white shadow">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                    </div>
                                    <span class="font-benzin text-xs font-bold mt-1 text-white">{{ $portfolio->title ?: 'Portofolio' }}</span>
                            ...
                            @if(!empty($portfolio->extra_meta['views']) || !empty($portfolio->subtitle))
                                <div class="absolute bottom-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-md bg-white text-slate-950 font-benzin font-extrabold text-[11px] sm:text-xs tracking-wider shadow-lg whitespace-nowrap z-10 border border-slate-200">
