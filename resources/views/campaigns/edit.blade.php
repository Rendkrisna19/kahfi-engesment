        ...
        <form method="POST" action="{{ route('campaigns.update', $campaign) }}" class="bg-surface rounded-2xl border border-border p-6 shadow-sm space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3" class="mt-1 block w-full border-border bg-body text-primary rounded-lg focus:border-brand-blue focus:ring-brand-blue shadow-sm">{{ old('deskripsi', $campaign->deskripsi) }}</textarea>
                </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 p-4 bg-body border border-border rounded-xl max-h-48 overflow-y-auto">
                        @forelse($admins as $admin)
                            <label class="flex items-center gap-3 cursor-pointer p-2.5 rounded-lg hover:bg-surface border border-transparent hover:border-border transition-colors">
                                <input type="checkbox" name="admin_ids[]" value="{{ $admin->id }}" {{ in_array($admin->id, old('admin_ids', $assignedAdminIds)) ? 'checked' : '' }} class="rounded border-border text-brand-blue focus:ring-brand-blue w-4 h-4">
                                <div class="truncate">
