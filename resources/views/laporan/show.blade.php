<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('laporan.index') }}" class="text-secondary hover:text-primary transition-colors bg-surface border border-border p-2 rounded-lg hover:shadow-sm">
    ...
    <div class="space-y-6">
        ...
        @if(in_array($link->status_scraping, ['Berhasil', 'Completed']))
            ...
            <!-- Charts Section -->
            @if(Auth::user()->role === 'Client' || Auth::user()->hasRole('Client'))
                <!-- Bar Chart (Full Width for Client) -->
            ...
            @else
                <!-- Charts Section with SAW Preference Score (Admin & Admin Master) -->
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-surface rounded-2xl border border-border p-6 shadow-sm flex flex-col justify-center items-center text-center">
                        ...
                        <p class="text-sm text-secondary mb-6">Simple Additive Weighting (Share 35%, Comment 25%, Like 20%, View 10%, Save 10%)</p>
        ...
        @else
            ...
            <div class="bg-surface rounded-2xl border border-border p-12 shadow-sm text-center">
                <div class="w-20 h-20 bg-brand-blue/10 rounded-full flex items-center justify-center mx-auto mb-4">
                    <svg class="w-10 h-10 text-brand-blue" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>
                <h3 class="text-xl font-bold text-primary mb-2">Detail Belum Tersedia</h3>
                <p class="text-secondary max-w-md mx-auto">Link ini belum selesai diproses atau scraping tidak berhasil.</p>
            </div>
        @endif
    </div>
</x-app-layout>
