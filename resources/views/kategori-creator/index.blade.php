<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-primary leading-tight">
            Kelola Kategori Creator
        </h2>
        <p class="text-sm text-secondary mt-1">Atur master data kategori untuk level/jenis konten creator.</p>
    </x-slot>
    ...
    <div class="space-y-6" x-data="{ isLoaded: false, isModalOpen: false, modalMode: 'create', modalData: {id: '', nama: ''} }" x-init="setTimeout(() => isLoaded = true, 500)">
        ...
        <div class="bg-surface rounded-2xl border border-border overflow-hidden shadow-sm">
            <div class="p-6 border-b border-border flex justify-between items-center">
                <h3 class="text-lg font-bold text-primary">Daftar Kategori Creator</h3>
            ...
            <div x-show="isLoaded" style="display: none;" class="overflow-x-auto">
                <table class="w-full text-sm text-left text-primary">
                    ...
                    <tbody class="divide-y divide-border">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <form action="{{ route('kategori-creator.destroy', $item) }}" method="POST" class="inline" onsubmit="return confirm('Hapus kategori ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-secondary">Belum ada kategori creator.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="isModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="isModalOpen" x-transition.scale.origin.bottom class="inline-block align-bottom bg-surface rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
                    <form :action="modalMode === 'edit' ? '{{ url('kategori-creator') }}/' + modalData.id : '{{ route('kategori-creator.store') }}'" method="POST">
                        @csrf
                        <template x-if="modalMode === 'edit'">
                            <input type="hidden" name="_method" value="PUT">
                        </template>
                        <div class="px-6 pt-6 pb-4">
                            <h3 class="text-lg leading-6 font-bold text-primary" id="modal-title" x-text="modalMode === 'edit' ? 'Edit Kategori Creator' : 'Tambah Kategori Creator'"></h3>
                            <div class="mt-4">
